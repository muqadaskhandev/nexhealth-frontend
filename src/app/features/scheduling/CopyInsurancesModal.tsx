...
export function CopyInsurancesModal({ onClose, onCopied }: { onClose: () => void; onCopied: () => void }) {
  ...
  async function handleCopy() {
    ...
    setError(null);
    try {
      const result = await staffApi.bookingInsurances.copy([...selectedLocationIds]);
      toastSuccess(
        `Copied insurances to ${selectedLocationIds.size} location${selectedLocationIds.size !== 1 ? "s" : ""} (${result.copied} added)`
      );
      onCopied();
      onClose();
    } catch (err: unknown) {
      const apiErr = err as { detail?: string };
      const msg = apiErr?.detail || "Could not copy insurances — please try again.";
      setError(msg);
      toastError(msg);
    } finally {
      setSubmitting(false);
      ...
      >
        ...
        <div className="px-6 pb-2 flex-1 overflow-y-auto">
          <p className="text-sm text-gray-500 mb-3">
            Copy your current location&apos;s insurance list to other locations. Existing insurers at each destination are kept.
          </p>
...
export function RestoreDefaultInsurancesModal({ onClose, onRestored }: { onClose: () => void; onRestored: () => void }) {
  const [submitting, setSubmitting] = useState(false);
  ...
  async function handleRestore() {
    ...
    setSubmitting(true);
    try {
      await staffApi.bookingInsurances.restoreDefaults();
      toastSuccess("Default insurance list restored");
      onRestored();
      onClose();
    } catch (err: unknown) {
      const apiErr = err as { detail?: string };
      toastError(apiErr?.detail || "Could not restore defaults — please try again.");
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white w-full max-w-md mx-4 rounded-2xl shadow-2xl p-6" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start gap-3 mb-4">
          <AlertTriangle size={20} className="text-amber-500 flex-shrink-0 mt-0.5" />
          <div>
            <h2 className="text-lg font-bold text-gray-900">Restore default insurances?</h2>
            <p className="text-sm text-gray-500 mt-1">
              This replaces your current insurance list with the NexHealth default carriers for this location.
            </p>
          </div>
        </div>
        <div className="flex items-center gap-4">
          <button
            ...
            disabled={submitting}
            className="px-5 py-2.5 bg-teal-500 hover:bg-teal-600 disabled:bg-gray-200 text-white text-sm font-semibold rounded-lg"
          >
            {submitting ? "Restoring…" : "Restore defaults"}
          </button>
